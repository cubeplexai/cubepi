...
from cubeloop.tracing.content import (
    messages_to_semconv,
...
from cubeloop.tracing.schema import (
    CUBELOOP_ABORTED,
    CUBELOOP_AGENT_SYSTEM_PROMPT_SHA256,
    CUBELOOP_AGENT_TOOLS,
    CUBELOOP_INPUT_MESSAGES_COUNT,
    CUBELOOP_LLM_RAW_REQUEST,
    CUBELOOP_LLM_RAW_RESPONSE,
    CUBELOOP_LLM_THINKING_LEVEL,
    CUBELOOP_OUTPUT_MESSAGES_COUNT,
    CUBELOOP_RUN_ID,
    CUBELOOP_RUN_OUTCOME,
    CUBELOOP_TOOL_BLOCK_REASON,
    CUBELOOP_TOOL_BLOCKED_BY_HOOK,
    CUBELOOP_TOOL_EXECUTION_MODE,
    CUBELOOP_TOOL_IS_ERROR,
    CUBELOOP_TOOL_TERMINATE,
    CUBELOOP_TURN_INDEX,
    CUBELOOP_TURN_STOP_REASON,
    CUBELOOP_TURN_TERMINATED_BY_TOOL,
    CUBELOOP_TURN_TOOL_CALLS_COUNT,
    ERROR_TYPE,
...
_active_run: contextvars.ContextVar[object | None] = contextvars.ContextVar(
    "cubeloop.tracing.active_run", default=None
)
...
class _RunState:
    ...
    # ``cubeloop.mcp._tracing.register_tool_span``. Passed back on
...
class Recorder:
    ...
    def _sweep_tool_span_tokens(self, run: "_RunState | None") -> None:
        ...
        in :mod:`cubeloop.mcp._tracing._active_entries`. Repeated aborts
        ...
        if run is None or not run.tool_span_tokens:
            return
        try:
            from cubeloop.mcp import _tracing as _mcp_tracing
    ...
    def _close_open_spans(self, run: "_RunState | None") -> None:
        ...
        Normal flow ends ``execute_tool`` / ``chat`` / ``cubeloop.turn`` /
        ...
        Each span gets ``cubeloop.aborted=true`` + ``error.type`` so the
        ...
        for span in list(run.tool_spans.values()):
            try:
                span.set_attribute(CUBELOOP_ABORTED, True)
                span.set_attribute(ERROR_TYPE, "cubeloop.aborted")
                span.end()
            except Exception:
                pass
        ...
        if run.chat_span is not None:
            try:
                run.chat_span.set_attribute(CUBELOOP_ABORTED, True)
                run.chat_span.set_attribute(ERROR_TYPE, "cubeloop.aborted")
                run.chat_span.end()
            except Exception:
                pass
            run.chat_span = None
        ...
        if run.turn_span is not None:
            try:
                run.turn_span.set_attribute(CUBELOOP_ABORTED, True)
                run.turn_span.end()
            except Exception:
                pass
            run.turn_span = None
        if run.agent_span is not None:
            try:
                run.agent_span.set_attribute(CUBELOOP_ABORTED, True)
                run.agent_span.end()
            except Exception:
                pass
    ...
    def _on_agent_start(self) -> None:
        ...
        try:
            ...
            # cubeloop.mcp._tracing — it now serves generic nested agents, not
            # just MCP clients.
            from cubeloop.mcp import _tracing as _mcp_tracing
        ...
        # Stamp any tags / metadata set via ``cubeloop.tracing.tracing_context``
        ...
        #
        # User metadata is namespaced under ``cubeloop.metadata.*`` so
        # that keys like ``run_id`` / ``turn_index`` / ``agent.tools``
        # — which the recorder owns under ``cubeloop.*`` — can never be
        # overwritten by caller-supplied values. Reserved cubepi
        # schema keys (especially ``cubeloop.run_id``, a per-span filtering
        # attribute) must stay recorder-controlled (codex P2 on PR #92).
        try:
            from cubeloop.tracing.context import _current_metadata, _current_tags

            tags = _current_tags()
            if tags:
                span.set_attribute("cubeloop.tags", tags)
            ...
            for key, value in metadata.items():
                ...
                # whole span.
                try:
                    span.set_attribute(f"cubeloop.metadata.{key}", value)
    ...
    def _on_agent_suspended(self) -> None:
        ...
        for span in list(run.tool_spans.values()):
            try:
                span.set_attribute(CUBELOOP_RUN_OUTCOME, "suspended")
                span.end()
            except Exception:
                pass
        run.tool_spans.clear()
        if run.chat_span is not None:
            try:
                run.chat_span.set_attribute(CUBELOOP_RUN_OUTCOME, "suspended")
                run.chat_span.end()
            except Exception:
                pass
            run.chat_span = None
        ...
        if run.turn_span is not None:
            try:
                run.turn_span.set_attribute(CUBELOOP_RUN_OUTCOME, "suspended")
                run.turn_span.end()
            except Exception:
                pass
            run.turn_span = None
        run.agent_span.set_attribute(CUBELOOP_RUN_OUTCOME, "suspended")
        run.agent_span.set_attribute(
            CUBELOOP_OUTPUT_MESSAGES_COUNT, len(run.output_messages)
        )
        run.agent_span.end()
    ...
    def _on_agent_end(self, event: AgentEndEvent) -> None:
        ...
        run.agent_span.set_attribute(
            CUBELOOP_OUTPUT_MESSAGES_COUNT, run.new_message_count
        )
    ...
    def _on_turn_start(self) -> None:
        ...
        run.turn_span = self._tracer.otel_tracer.start_span(
            ...
            context=ctx,
            attributes={
                CUBELOOP_TURN_INDEX: run.turn_index,
    ...
    def _on_turn_end(self, event: TurnEndEvent) -> None:
        ...
        if stop_reason:
            run.turn_span.set_attribute(CUBELOOP_TURN_STOP_REASON, stop_reason)
        tool_calls_count = sum(
            1
            for c in getattr(msg, "content", [])
            if getattr(c, "type", "") == "tool_call"
        )
        run.turn_span.set_attribute(CUBELOOP_TURN_TOOL_CALLS_COUNT, tool_calls_count)
        if run.turn_terminated_by_tool:
            run.turn_span.set_attribute(CUBELOOP_TURN_TERMINATED_BY_TOOL, True)
        ...
        # cubeloop.aborted on the invoke_agent root.
        if stop_reason == "error":
            ...
            run.turn_span.set_attribute(ERROR_TYPE, "cubeloop.error")
    ...
    def _on_tool_exec_start(self, event: ToolExecutionStartEvent) -> None:
        ...
        if tool_obj is not None:
            ...
            attrs[CUBELOOP_TOOL_EXECUTION_MODE] = mode
        span = self._tracer.otel_tracer.start_span(
        ...
        # ``cubeloop.mcp._tracing`` via a per-task contextvar so an MCP
    ...
    def _on_tool_exec_end(self, event: ToolExecutionEndEvent) -> None:
        ...
        if span is None:
            return
        span.set_attribute(CUBELOOP_TOOL_IS_ERROR, event.is_error)
        if self._record_content and event.result is not None:
            self._set_content_attr(
                span, GEN_AI_TOOL_CALL_RESULT, _coerce_dict(event.result)
            )
        if event.terminate:
            span.set_attribute(CUBELOOP_TOOL_TERMINATE, True)
            run.turn_terminated_by_tool = True
        if event.blocked_by_hook:
            span.set_attribute(CUBELOOP_TOOL_BLOCKED_BY_HOOK, True)
            if event.block_reason is not None:
                span.set_attribute(CUBELOOP_TOOL_BLOCK_REASON, event.block_reason)
        if event.is_error:
            span.set_status(Status(StatusCode.ERROR, "tool error"))
            err_type = (
                "cubeloop.tool.blocked_by_hook"
                if event.blocked_by_hook
                else "cubeloop.tool.error"
            )
            ...
            run.agent_span.set_attribute(
                ...
                int(
                    (run.agent_span.attributes or {}).get(
                        CUBELOOP_INPUT_MESSAGES_COUNT, 0
                    )
                )
                + 1,
            )
    ...
    def _on_provider_request(self, payload: dict, model: "Model") -> None:
        ...
        # Thinking level on cubeloop.* namespace.
        thinking = payload.get("thinking")
        if isinstance(thinking, dict) and thinking.get("type") == "enabled":
            attrs[CUBELOOP_LLM_THINKING_LEVEL] = "on"
        elif thinking is None and (
            payload.get("reasoning", {}).get("effort")
            if payload.get("reasoning")
            else None
        ):
            attrs[CUBELOOP_LLM_THINKING_LEVEL] = payload["reasoning"]["effort"]
        ...
        if attribute_root:
            ...
            if isinstance(tools, list) and tools:
                run.agent_span.set_attribute(
                    CUBELOOP_AGENT_TOOLS, [_safe_tool_name(t) for t in tools]
                )
        ...
        if self._record_content:
            ...
            self._set_content_attr(chat_span, CUBELOOP_LLM_RAW_REQUEST, payload)
        ...
        try:
            if body is not None:
                ...
                if self._record_content:
                    ...
                    self._set_content_attr(span, CUBELOOP_LLM_RAW_RESPONSE, body)
            ...
            if exc is None and body is None and self._agent_signal_is_set():
                ...
                # Status UNSET, set cubeloop.aborted + error.type.
                #
                ...
                span.set_attribute(ERROR_TYPE, cubeloop_error_type_for(exc))
    ...
    def _maybe_record_system_prompt_hash(self, payload: dict, run: _RunState) -> None:
        if (run.agent_span.attributes or {}).get(CUBELOOP_AGENT_SYSTEM_PROMPT_SHA256):
            return
        text = _extract_system_prompt(payload)
        if not text:
            return
        digest = hashlib.sha256(text.encode("utf-8")).hexdigest()[:16]
        run.agent_span.set_attribute(CUBELOOP_AGENT_SYSTEM_PROMPT_SHA256, digest)
...
def _safe_tool_name(t: Any) -> str:
    ...
    - Anthropic / cubeloop.AgentTool: top-level ``{"name": ...}``
