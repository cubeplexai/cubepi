"""Per-run tagging / metadata context for cubeloop.tracing.

Sets a contextvar-scoped ``tags`` / ``metadata`` payload that the
:class:`~cubeloop.tracing.recorder.Recorder` reads on ``AgentStartEvent``
...
- Tags use a single attribute ``cubeloop.tags`` (tuple of strings).
- User metadata is namespaced under ``cubeloop.metadata.*`` so that
  recorder-owned schema keys (``cubeloop.run_id``,
  ``cubeloop.turn.index``, …) can never be overridden by
...
_run_metadata: contextvars.ContextVar[dict[str, Any]] = contextvars.ContextVar(
    "cubeloop.tracing.run_metadata", default={}
)

_run_tags: contextvars.ContextVar[tuple[str, ...]] = contextvars.ContextVar(
    "cubeloop.tracing.run_tags", default=()
)
...
def _current_tags() -> tuple[str, ...]:
    ...
    Called by :class:`~cubeloop.tracing.recorder.Recorder` on
