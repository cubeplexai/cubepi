...
if TYPE_CHECKING:
    ...
    from cubeloop.providers.base import BoundModel, Message
...
class Tracer:
    ...
    def __init__(
        ...
        *,
        ...
        self._otel_tracer = self._provider.get_tracer(
            ...
            instrumenting_library_version=cubeloop.__version__,
    ...
    def attach(self, agent: "Agent") -> Callable[[], Any]:
        ...
        :mod:`cubeloop.mcp._tracing` so MCP CLIENT spans flow through
    ...
    async def oneshot(
        ...
        *,
        ...
        from cubeloop.providers.base import BaseProvider as _BaseProvider
        from cubeloop.tracing.recorder import Recorder, _RunState, _active_run
        ...
        root_attrs: dict[str, Any] = {
            ...
            "cubeloop.oneshot.operation": operation,
        }
        ...
        for k, v in (metadata or {}).items():
            root_attrs[f"cubeloop.metadata.{k}"] = v
        # Also expose operation under cubeloop.metadata.* so the
        # `cubepi trace ls --meta oneshot_operation=...` filter works
        # (the CLI's --meta filter only reads cubeloop.metadata.* attrs).
        root_attrs["cubeloop.metadata.oneshot_operation"] = operation
        ...
        # under the root (no cubeloop.turn wrapper — one-shot has no loop).
        run = _RunState(
            run_id=run_id,
        ...
        try:
            ...
            # tracked as cubeloop.aborted (UNSET status, semconv-aligned)
            # rather than ERROR.
            try:
                from opentelemetry.trace import Status, StatusCode

                from cubeloop.tracing.schema import CUBELOOP_ABORTED, ERROR_TYPE

                import asyncio as _asyncio_for_cancel

                if isinstance(_exc, _asyncio_for_cancel.CancelledError):
                    root_span.set_attribute(CUBELOOP_ABORTED, True)
                    root_span.set_attribute(ERROR_TYPE, "cubeloop.aborted")
                else:
...
def _log_tracing_warning(message: str, exc: BaseException) -> None:
    ...
    try:
        logging.getLogger("cubeloop.tracing").warning(
