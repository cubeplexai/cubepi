...
DEFAULT_DIR = Path("./cubeloop-traces")
_MIN = datetime.min.replace(tzinfo=timezone.utc)

# Run-scoped metadata is stamped as ``cubeloop.metadata.<key>`` (0.13: cubepi.metadata.*).
_META_PREFIXES = ("cubeloop.metadata.", "cubepi.metadata.")
...
def _run_metadata(spans: list[Span]) -> dict[str, str]:
    """The ``cubeloop.metadata.*`` attributes off the trace's root span, with the
...
def resolve_run(arg: str, directory: Path) -> list[Path]:
    ...
    raise RunResolutionError(
        f"no trace file for trace {arg!r} under {directory} (try `cubeloop trace ls`)"
    )
