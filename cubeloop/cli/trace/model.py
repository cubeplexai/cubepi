...
class Span:
    ...
    def is_aborted(self) -> bool:
        # The recorder sets a boolean cubeloop.aborted attribute (and also an
        # error.type="cubeloop.aborted" string). The boolean is the canonical
        # signal; OTel status stays UNSET for aborts, so don't gate on status.
        return schema.is_aborted_attrs(self.attributes)

    @property
    def run_id(self) -> str | None:
        value = schema.attr(self.attributes, schema.CUBELOOP_RUN_ID)
    ...
    def operation(self) -> str | None:
        ...
        so classify on this attribute, not on ``name``. The cubeloop.turn span
