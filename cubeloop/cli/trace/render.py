...
from cubeloop.cli.trace.model import Span, TreeNode
from cubeloop.cli.trace.stats import StatRow
...
def _require_rich() -> None:
    try:
        ...
        raise RichMissingError(
            "this command needs rich. Install it via: pip install cubeloop[trace-cli]"
        ) from exc
...
def render_runs(runs: list[RunSummary], show_meta: list[str] | None = None) -> None:
    ...
    console = _console()
    table = Table(title="cubeloop runs")
