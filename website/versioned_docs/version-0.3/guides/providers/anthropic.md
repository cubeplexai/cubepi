...
`provider` string is a free-form label used by CubePi internals — keep
...
```python
AnthropicProvider(api_key=…, cache_retention="long")  # 1-hour TTL
AnthropicProvider(api_key=…, cache_retention="none")  # disable entirely
```
