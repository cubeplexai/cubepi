...
def upgrade():
    op.create_table(...)        # auto-generated by Alembic from cubepi_metadata
    write_schema_version_op(op) # records EXPECTED_SCHEMA_VERSION
```

When CubePi later upgrades and bumps `EXPECTED_SCHEMA_VERSION`, you
generate a new revision and call `write_schema_version_op` again.
