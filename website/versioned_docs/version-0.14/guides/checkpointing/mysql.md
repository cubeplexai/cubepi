...
`cubeloop_runs`, `cubeloop_hitl_answers`, and `cubeloop_schema_version`. Like
...
The `cubeloop_messages` table is `KEY`-partitioned, which SQLAlchemy's
...
`parent_thread_id` / `forked_at_seq` columns on `cubeloop_threads`
record fork lineage; `cubeloop_runs` (added in v4) tracks per-run
...
0.14 renames the MySQL tables from `cubepi_*` to `cubeloop_*`.
...
- **`CubeloopSchemaUninitialized`** — Your DB is empty, your migrations
  didn't run, or the `cubeloop_schema_version` table is malformed. Apply
