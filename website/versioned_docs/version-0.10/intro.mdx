...
- **Eight-hook middleware.** `transform_context`, `convert_to_llm`,
  `transform_system_prompt`, `before_tool_call`, `after_tool_call`,
  `should_stop_after_turn`, `after_model_response`, `on_run_end` — each
  with explicit composition rules. No mystery node ordering.
...
You are reading the **CubePi 0.10** release docs. This is the latest
stable documentation set for installing CubePi and building against the
published 0.10 API.

CubePi 0.10 adds `DeferredToolGroup` for progressive MCP tool disclosure
(hide schemas behind a catalog, expand on demand via the `load_tools`
tool), `BoundModel.generate_structured()` for tool-based structured
output validated against a Pydantic model, a `tool_choice` parameter on
the `Provider` protocol (`"auto"` / `"required"` / `"none"` / tool name),
and `GoalMiddleware` for autonomous goal-driven runs judged by a
separate evaluator model. `on_run_end` now fires on every outer-loop
iteration instead of once per `prompt()` call, and `Recorder` / `Meter`
now subscribe to every leg of a `FallbackBoundModel` chain so post-failover
calls are no longer invisible to provider-level observability (see
