...
version picker, top-right](pathname:///)). CubePi 0.7 adds provider-level
`generate()`, compaction and subagent middleware, `on_run_end`, typed provider
errors, `Tracer.oneshot()`, and checkpointed HITL run IDs.
