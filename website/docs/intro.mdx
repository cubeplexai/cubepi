...
performance, readability, and production-grade persistence. It models an
agent run as a linear `while` loop you can trace and debug — no DAGs, no
graph DSL, no surprise control flow. You can read the entire core loop in
five minutes.
