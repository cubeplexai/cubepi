...
const EN: CompareContent = {
  them: 'CrewAI',
  title: 'CubeLoop vs CrewAI — a leaner Python agent framework',
  description:
    'CubeLoop vs CrewAI: a side-by-side comparison. CrewAI models agents as role-playing crew members with assigned tasks. CubeLoop models the agent as a plain async while-loop with typed middleware, append-only checkpointing, and 3 core dependencies.',
  keywords:
    'CubeLoop vs CrewAI, CrewAI alternative, Python agent framework, async agent framework, CrewAI vs CubeLoop, multi-agent framework, crewai 替代品',
  h1: 'CubeLoop vs CrewAI',
  intro: [
    'CrewAI organises your agent system around role-playing metaphors — crews, agents with roles and backstories, and tasks that agents are assigned to. CubeLoop starts from a different premise: an agent is a plain async while-loop that calls the model, runs tools, and repeats.',
    'If the role-based abstraction is solving a problem you do not have, CubeLoop removes that layer. Here is how the two compare.',
  ],
  ...
  sections: [
    {
      ...
      body: [
        'CrewAI\'s "role", "goal", and "backstory" fields are a structured way to write a system prompt. When you have one agent doing one job, the role metaphor adds ceremony without adding capability. In CubeLoop, `system_prompt` is a string you own completely — no framework vocabulary to learn, no structure to maintain.',
        'For genuine multi-agent scenarios, CubeLoop provides a `SubagentsMiddleware` that spawns child agents and wires their results back to the parent — without the crew/process/task object model.',
      ],
    },
    {
      h2: 'Persistence that does not require you to wire it yourself',
      body: [
        'CrewAI does not ship a built-in checkpointing layer. If you want conversations to survive restarts you add your own database logic. CubeLoop\'s append-only checkpointing ships out of the box — `SQLiteCheckpointer`, `PostgresCheckpointer`, and `MySQLCheckpointer` each write only the new messages from each turn, so write cost stays O(1) regardless of how long the thread grows.',
      ],
    },
    {
      h2: 'When CrewAI is the better fit',
      body: [
        'CrewAI is a reasonable choice if your workflow maps naturally onto the crew metaphor — a team of specialist agents each assigned a discrete task in a pipeline. If you are building a single conversational agent, a stateful assistant, or anything that needs production-grade persistence or OTel observability, CubeLoop is the leaner path.',
      ],
    },
  ],
  cta: [
...
const ZH: CompareContent = {
  them: 'CrewAI',
  title: 'CubeLoop vs CrewAI — 更精简的 Python Agent 框架',
  description:
    'CubeLoop 与 CrewAI 对比：CrewAI 将 Agent 建模为具有角色和背景故事的"角色扮演"团队成员。CubeLoop 将 Agent 建模为普通的异步 while 循环，具备类型化中间件、追加式 checkpointing 和 3 个核心依赖。',
  keywords:
    'CubeLoop vs CrewAI, CrewAI 替代品, Python Agent 框架, 异步 Agent 框架, CrewAI vs CubeLoop, 多 Agent 框架, crewai 替代品',
  h1: 'CubeLoop vs CrewAI',
  intro: [
    'CrewAI 围绕角色扮演的隐喻组织 Agent 系统 —— 团队（crew）、带有角色和背景故事的 Agent，以及分配给 Agent 的任务。CubeLoop 从不同的前提出发：Agent 是一个普通的异步 while 循环，调用模型、执行工具、循环往复。',
    '如果角色抽象并不能解决你的问题，CubeLoop 直接省去这一层。以下是两者的详细对比。',
  ],
  tableHeading: '并排对比',
  rows: [
  ...
  sections: [
    {
      ...
      body: [
        'CrewAI 的"角色"、"目标"和"背景故事"字段本质上是一种结构化写 system prompt 的方式。当你只有一个 Agent 做一件事时，角色隐喻增加的是仪式感而非能力。在 CubeLoop 中，`system_prompt` 就是你完全掌控的一个字符串 —— 没有框架词汇要学，没有结构要维护。',
        '对于真正的多 Agent 场景，CubeLoop 提供 `SubagentsMiddleware`，可以派生子 Agent 并将其结果回传给父 Agent —— 无需 crew/process/task 对象模型。',
      ],
    },
    {
      h2: '开箱即用的持久化，无需自己接线',
      body: [
        'CrewAI 没有内置的 checkpointing 层。如果你希望对话在重启后依然存在，需要自己添加数据库逻辑。CubeLoop 的追加式 checkpointing 开箱即用 —— `SQLiteCheckpointer`、`PostgresCheckpointer` 和 `MySQLCheckpointer` 每轮只写入新消息，无论对话多长，写入成本保持 O(1)。',
      ],
    },
    {
      h2: 'CrewAI 更适合的场景',
      body: [
        '如果你的工作流自然地映射到 crew 的隐喻 —— 一组专家 Agent 各自负责流水线中的一个离散任务 —— CrewAI 是合理的选择。如果你在构建单一的对话 Agent、有状态的助手，或任何需要生产级持久化或 OTel 可观测性的系统，CubeLoop 是更精简的路径。',
      ],
    },
  ],
  cta: [
