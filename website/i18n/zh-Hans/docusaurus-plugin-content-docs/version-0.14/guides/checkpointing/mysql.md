...
Checkpointer 需要这些表：`cubeloop_threads`、`cubeloop_messages`、`cubeloop_runs`、`cubeloop_hitl_answers` 和 `cubeloop_schema_version`。与 Postgres 一样（与 SQLite 不同），CubeLoop **不会为你创建这些表**——它会在 `__aenter__` 时验证这些表是否存在并具有预期的 `schema_version`。
...
`cubeloop_messages` 表使用 `KEY` 分区，SQLAlchemy 的自动生成无法表达这一点，因此需要使用提供的辅助函数手动追加分区子句和 schema 版本写入：
...
def upgrade():
    op.create_table("cubeloop_threads", ...)
    op.create_table(
        "cubeloop_messages", ...,
        mysql_engine="InnoDB",
    )
    ...
    op.execute(
        "ALTER TABLE cubeloop_messages " + messages_partition_clause()
    )
    op.create_table("cubeloop_schema_version", ...)
...
`cubeloop_threads` 上的 `parent_thread_id` / `forked_at_seq` 列
用于记录 fork 谱系；v4 新增的 `cubeloop_runs` 表用于追踪每个 run
...
0.14 把 MySQL 表从 `cubepi_*` 重命名为 `cubeloop_*`。
