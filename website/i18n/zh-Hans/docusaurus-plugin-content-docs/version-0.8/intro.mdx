...
你正在浏览的是 **CubePi 0.8** 发布文档，这是基于已发布 0.8 API 的最新稳定
文档。

CubePi 0.8 新增会话分叉（`Agent.fork()` / `Agent.fork_once()`）、基于 run
的 checkpointing（全后端 schema v3→v4）、`@tool` 装饰器（便捷工具定义），以
及 `Agent.prompt()` 现在会返回 `run_id`。如需查看 `main` 分支上的未发布变更，
请通过版本选择器切换到 **Next** 文档。
