---
name: cubeloop-trace
description: Use when debugging a cubeloop/cubebox agent run — a missing final reply, a tool that misbehaved, a 4xx from the model, wrong token/cache numbers, or "why did the agent do that?". Reads the JSONL traces cubeloop writes per run via the `cubeloop trace` CLI (ls / view / follow / stats) to inspect the span tree, errors, tool inputs/outputs, and token usage. Triggers on phrases like "查一下这个 run", "trace 一下", "为什么最后一轮没回复", "工具结果不对", "看 trace".
---

# Debugging CubeLoop runs with `cubeloop trace`

cubeloop can record every agent run as an OpenTelemetry span tree, written to
local JSONL files. The `cubeloop trace` CLI reads those files so you can see
...
- `tracing.directory: ./cubeloop-traces`  (default; relative to the backend cwd)
...
dirs; the CLI merges them. (`cubeloop.run_id` is still recorded as a per-span
...
cubeloop installed (the `trace-cli` extra provides it):

```bash
uv run cubeloop trace --help
```

`--dir` defaults to `./cubeloop-traces`; pass `--dir <path>` if your traces live
...
uv run cubeloop trace view 1cd97cdb
```
...
└── invoke_agent  14425.8ms  [0x1cd97cdb]
    ├── cubeloop.turn  1283.1ms  [0x5cfda93e]
...
Read the tree top-down: `invoke_agent` (one run) → `cubeloop.turn` (one agent
loop turn) → `chat <model>` (an LLM call, with `tok <input>/<output>`) and
`execute_tool <name>` (a tool call). A **subagent** appears as
`execute_tool subagent` with the subagent's own `invoke_agent → cubeloop.turn →
...
uv run cubeloop trace ls --meta conversation_id=conv_123
uv run cubeloop trace ls --meta user_id=usr_9 --meta org_id=org_1
uv run cubeloop trace stats --by model --meta user_id=usr_9

# Show metadata as ls columns (display, not filter):
uv run cubeloop trace ls --show-meta conversation_id,user_id
```
...
tasks like memory consolidation. The span tree is flat — no `cubeloop.turn`
...
- `cubeloop.oneshot.operation` — the operation name (e.g. `"consolidate_memory"`)
- `cubeloop.metadata.*` — caller-supplied metadata (e.g. `conversation_id`)

Find oneshot traces by operation or conversation:

```bash
# All consolidation runs  (operation is exposed as cubeloop.metadata.oneshot_operation
# so the --meta filter can reach it; cubeloop.oneshot.operation is also set for dashboards)
uv run cubeloop trace ls --meta oneshot_operation=consolidate_memory

# Oneshot traces for a specific conversation (alongside its agent runs)
uv run cubeloop trace ls --meta conversation_id=conv-1fwZQ8u3ZDukx3

# Stats on consolidation token usage
uv run cubeloop trace stats --by model --meta oneshot_operation=consolidate_memory
```
...
uv run cubeloop trace convert <run_id>

# Pick a specific call by span_id prefix (copy [0x…] from `view`)
uv run cubeloop trace convert <run_id> --span 0xbb7eb1

# Shell-executable curl (uses $BASE_URL / $API_KEY env vars)
uv run cubeloop trace convert <run_id> --span 0xbb7eb1 --format curl

# Anthropic Messages API shape
uv run cubeloop trace convert <run_id> --format anthropic
```
...
```python
Tracer(record_content=True, record_stream=True, stream_dir="./cubeloop-traces", …)
```
...
grep '"type": "toolcall' cubeloop-traces/<date>/<run_id>.stream.jsonl | python -m json.tool

# Check for duplicate toolcall_end (double finish_reason bug pattern):
grep '"toolcall_end"' cubeloop-traces/<date>/<run_id>.stream.jsonl | wc -l   # expect 1
```
...
- **Subagent missing from the tree?** It should nest under
  ...
  was recorded with a cubeloop new enough to shard by `trace_id` (older runs
...
Skill source and CubeLoop source: https://github.com/cubeplexai/cubeloop  
If the docs or skill are unclear, read the source — it's the authoritative reference.  
Bugs or missing coverage: open an issue or PR at https://github.com/cubeplexai/cubeloop/issues
